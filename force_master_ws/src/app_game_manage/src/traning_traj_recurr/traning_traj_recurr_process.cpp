...
std::ofstream train_traj_recurr("../../../docs/traj_recurr.csv", fstream::out|ios_base::trunc);

bool TrainingTrajRecurr::generateJointPathFromJointFile()
{
    /* end to start*/
...
bool TrainingTrajRecurr::moveToEndJoint(ur_data_type::Joint actual_joint)
{
	if (joint_path_.size() < 1)
	{
		printf("TrainingTrajRecurr: joint path is null\n");
		return false;
	}

	if (isReachedJointPosition(actual_joint, generate_path_.getJointEndPosition()))
	{
		printf("TrainingTrajRecurr: reached end joint position\n");
		return true;
	}
	...
	{
		printf("TrainingTrajRecurr: move to end joint position failed.\n");
		return false;
	}
    printf("TrainingTrajRecurr: jointMove success\n");
	return true;
}

bool TrainingTrajRecurr::isReachedJointPosition(ur_data_type::Joint actual_joint, ur_data_type::Joint desire_joint)
{
	for (int j = 0; j != 6; ++j)
	{
		if (fabs(actual_joint.jVal[j] - desire_joint.jVal[j]) > 0.15)
			return false;
	}
...
bool TrainingTrajRecurr::moveFromEndToStart()
{
	...
	string script;
	robot_rt_->generateServoJScript(joint_path_end2start, fabs(passive_speed_) / 2.0, fabs(passive_speed_), script);
...
bool TrainingTrajRecurr::moveFromStartToEnd()
{
	std::vector<ur_data_type::Joint> joint_position_list = generate_path_.getJointPath();
	if (joint_position_list.empty())
	{
	...
	string script;
	robot_rt_->generateServoJScript(joint_position_list, fabs(passive_speed_) / 2.0, passive_speed_, script);
	if (!robot_rt_->sendScript(script))
	{
		printf("TrainingTrajRecurr: sendScript failed\n");
		return false;
