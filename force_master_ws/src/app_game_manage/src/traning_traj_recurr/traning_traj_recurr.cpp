...
bool TrainingTrajRecurr::initParams(app_game_model::AppGameParams params)
{
	...
	{
		printf("TrainingTrajRecurr: generate joint path failed from joint file!\n");
		return false;
	}
	Joint joint = generate_path_.getJointEndPosition();
	printf("TrainingTrajRecurr: end joint: %lf, %lf, %lf, %lf, %lf, %lf\n",
...
bool TrainingTrajRecurr::start(ControlArmRealtime* control_arm_rt, FTSensor* ft_sensor)
{
    ...
    printf("TrainingTrajRecurr: moving to end joint position.");
    ...
    printf("TrainingTrajRecurr: force level = %d, speed_level = %d, repeated_count = %d\n",
...
void TrainingTrajRecurr::encodeRuntimeInfo(char* buf, int &buf_size)
{
    return;
}

bool TrainingTrajRecurr::decodeRuntimeCmd(char* buf, int buf_size)
{
    return true;
}
